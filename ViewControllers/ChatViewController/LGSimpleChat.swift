...
class LGChatMessageCell : UITableViewCell {
    ...
    struct Appearance {
        ...
        static var userColor = UIColor.lightGrayColor()
        static var font: UIFont = UIFont.systemFontOfSize(12.0)
    }
  ...
  private lazy var userImageView: UIImageView = {
    ...
    let halfWidth = CGRectGetWidth(userImageView.bounds) / 2.0
    let halfHeight = CGRectGetHeight(userImageView.bounds) / 2.0
    
    // Center the imageview vertically to the textView when it is singleLine
    let textViewSingleLineCenter = self.textView.textContainerInset.top + (Appearance.font.lineHeight / 2.0)
    
    var maxWidth = CGRectGetWidth(self.bounds)
    userImageView.center = CGPointMake(maxWidth - 20 , textViewSingleLineCenter)
    userImageView.backgroundColor = UIColor.lightTextColor()
    ...
    private func styleTextViewForSentBy(sentBy: LGChatMessage.SentBy) {
        ...
        case .User:
            ...
            if self.userImageView.image != nil {
              ...
              self.textView.center.x -= CGRectGetWidth(self.userImageView.bounds) + padding
          }
        }
    }
}
...
class LGChatInput : UIView, LGStretchyTextViewDelegate {
    ...
    struct Appearance {
        ...
        static var tintColor = UIColor.whiteColor()
        static var backgroundColor = UIColor(red: 71.0/255.0, green: 168.0/255.0, blue: 184.0/255.0, alpha: 1.0)
        static var textViewFont = UIFont.systemFontOfSize(17.0)
        static var textViewTextColor = UIColor.whiteColor()
        static var textViewBackgroundColor = UIColor(red: 65.0/255.0, green: 155.0/255.0, blue: 169.0/255.0, alpha: 1.0)
    }
    ...
    */
    ...
    var textViewInsets = UIEdgeInsets(top: 10, left: 10, bottom: 10, right: 10)
    ...
    func setupSendButton() {
       ...
       // self.sendButton.setTitle("Send", forState: .Normal)
        self.sendButton.setImage(UIImage(named: "Send.png"), forState: UIControlState.Normal)
        self.sendButton.addTarget(self, action: "sendButtonPressed:", forControlEvents: .TouchUpInside)
        self.sendButton.bounds = CGRect(x: 0, y: 0, width:20, height:20)
